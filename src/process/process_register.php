...
if (!$_SERVER["REQUEST_METHOD"] == "POST" || !isset($_POST['csrf_token'], $_SESSION['csrf_token']) || $_POST['csrf_token'] != $_SESSION['csrf_token']) {
    ...
    } elseif (strlen($customer_email) < 10 || strlen($customer_email) > 30) {
        display_errorMsg("Email must be between 10 and 30 characters long.");
    }

    // Validate Address
    if (strlen($customer_address) < 10 || strlen($customer_address) > 30) {
        display_errorMsg("Address must be between 10 and 30 characters long.");
